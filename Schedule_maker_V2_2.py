...
def write_cumul(scheds, names):
    File_Name = "Cumule.csv"
    with open(File_Name, "w") as file:
        for i in range(len(scheds)):
            if scheds[i] != 0:
                line = names[i] + ","
                for integer in scheds[i]:
                    line += str(integer)
                    line += ","
                line += "\n"
                file.write(line)
...
def multi_graph(Data, Names, save_fig=False):
    ...
    Rooms = ['C16-X', 'C2B-B', 'C2F-F2', 'Unknown', 'C2E-S3', 'C2E-JETE', 'C2F-F1', 'C2E-S4', 'C2D-D', 'CT1-B', 'C15-Y',
             'CT2-A', 'C11-U']
...
def compare_schedule(file_real, file_pred, MORNING, BREAK, AFTER, name1, name2, display=False, save_file=False):
    ...
    data_cumul = [data[0], data[1], data[2]]
    names_cumul = ["Need", name1, name2]
